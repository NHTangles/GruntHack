GAMEDIR = grunthackdir

CFLAGS = -g
CXXFLAGS = -g

GAMELIBS = -lncurses -ldl
...
.PHONY: all
all: src/grunthack util/recover dat/ghdat dat/license
...
install: all
	mkdir -p $(GAMEDIR)
	install src/grunthack $(GAMEDIR)
	install util/recover $(GAMEDIR)
	install -m 644 dat/ghdat dat/license $(GAMEDIR)
	touch $(GAMEDIR)/perm
...
SRCOBJ = allmain.o alloc.o apply.o artifact.o attrib.o ball.o bones.o	\
         ...
         uhitm.o vault.o version.o vision.o weapon.o were.o wield.o	\
...
WINCURSESOBJ = cursdial.o cursinit.o cursmain.o cursmesg.o cursstat.o	\
               cursmisc.o curswins.o cursinvt.o
UNCURSEDOBJ = libuncursed.o plugins.o
UNCURSEDPLUGINOBJ = tty.o wrap_tty.o

GAME_O = $(SRCOBJ:%.o=src/%.o) $(SYSUNIXOBJ:%.o=sys/unix/%.o)	\
         $(SYSSHAREOBJ:%.o=sys/share/%.o)			\
         $(WINTTYOBJ:%.o=win/tty/%.o)				\
         $(UNCURSEDOBJ:%.o=libuncursed/src/%.o)			\
         $(UNCURSEDPLUGINOBJ:%.o=libuncursed/src/plugins/%.o)	\
         $(WINCURSESOBJ:%.o=win/curses/%.o)
src/grunthack: $(GAME_O)
	$(CC) $(LDFLAGS) $^ $(LDLIBS) $(GAMELIBS) -o $@
AUTO_BIN += src/grunthack
...
QUEST_DES = Arch.des Barb.des Caveman.des 	\
            Healer.des Knight.des Monk.des	\
            Priest.des Ranger.des Rogue.des	\
            Samurai.des Tourist.des Valkyrie.des Wizard.des

SPEC_DES = bigroom.des castle.des		\
           endgame.des gehennom.des knox.des		\
           medusa.des mines.des oracle.des sokoban.des	\
           tower.des yendor.des
...
DAT_GHDAT = cmdhelp data dungeon help hh history opthelp options	\
            oracles quest.dat rumors wizhelp
dat/ghdat: util/dlb $(DAT_GHDAT:%=dat/%) $(ALL_TAG)
	cd dat && ../util/dlb cf ghdat $(DAT_GHDAT) *.lev
AUTO_DAT += dat/ghdat
